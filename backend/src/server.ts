...
async function main() {
  // In Cloudflare mode the database lives in D1, reached over HTTP through
  // the Worker (see database/dbClient.ts) — no local SQLite file to
  // initialize. Everywhere else (local dev, the VPS deployment), load it up
  // front so the schema/WAL setup in localSqlite.ts runs once at boot, same
  // as before this file was restructured for the dual-deployment split.
  if (!isCloudflareMode) {
    await import("./database/localSqlite");
  }
